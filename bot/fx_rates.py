...
async def _get_uah_rates() -> tuple[str, dict[str, Decimal]]:
    async with httpx.AsyncClient(timeout=10) as client:
        response = await client.get(_NBU_ENDPOINT, params={"json": ""})
...
async def get_latest_rates(base_currency: str) -> FxSnapshot:
    ...
    date_text, rates_to_uah = await _get_uah_rates()
